...
=== start docker container
  ...
  $ docker run --rm -p 4000:4000 -v "$PWD":/app masaomi/local-sushi-docker-image:fastqcapp-setup

=== star workflow manager

(from another terminal)

  $ docker ps -a
  (Check the container ID)
  $ docker exec -it [conteriner ID] bash
  (login the container)
  # cd workflow_manager/
...
* [http://hostname:4000](http://hostname:4000)
